...
if($eDate=="")
{
    $sql_query = "SELECT * FROM  tbl_event WHERE(event_date >='$sDate')ORDER BY event_date DESC;";
...
elseif($sDate=="")
{
    $sql_query = "SELECT * FROM  tbl_event WHERE(event_date <='$eDate')ORDER BY event_date DESC;";
    $result = mysqli_query($conn, $sql_query);
    $response = array();
    $count = 0;
    while($row = mysqli_fetch_array($result)){
    ...
    {
        $sql_query = "SELECT * FROM  tbl_event WHERE(event_date >='$sDate' AND event_date <='$eDate')ORDER BY event_date DESC;";
    $result = mysqli_query($conn, $sql_query);
    $response = array();
    $count = 0;
    while($row = mysqli_fetch_array($result)){
    ...
    //,"skills"=>$row["skils"],"doj"=>$row["doj"],"premium"=>$row["premium"]
?>