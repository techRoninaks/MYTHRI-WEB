...
function dataUpdate(pageName){
    ...
    if(confirm("Confirm Upload?")){
        ...
        xhttp.onreadystatechange  =  function() {
            if (this.readyState == 4 && this.status == 200) {
                if(xhttp.responseText == '1'){
                    ...
                    postimage = "";
                    cancelFunc();
                } else if(parseInt(xhttp.response)<0 && pageName == "task"){
                    if(confirm("Task already assigned! Switch to editing?")){
                        fetchData(Math.abs(parseInt(xhttp.responseText)),"task");
                    } else {
                        cancelFunc();
                    }
                }else{
        ...
        xhttp.open("POST", "assets/php/"+phpFile+".php", true);
        xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhttp.send("jsonObj="+jSONObj);
    }  
     else {
        return;
    }
